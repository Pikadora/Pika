...
Каждое уведомление имеет структуру: дата и время - главный процесс - статус сообщения - адрес сервера - сообщение.

Есть возможность запуска модуля в трех вариантах:
1. режим *Демон*, запуск модуля в фоновом режиме с повтором через некоторое время(согласно концигурационному файлу) если проверки 
   проходят успешно;
...
При выявлении ситуаций, отличающихся от нормы, модуль завершает свою работу с определенным кодом.
...
'''
В конфигурационном файле у каждого сервера прописан список установленных на нем критических сервисов, который поступает в 
качестве аргумента к функции.
Вторым аргументом является адрес сервера, для удобного отслеживания выполнения проверок на нескольких серверах.
...
2. Статус *отключен*.
Все сервисы при статусе *отключен* выводит критичное сообщение с адресом хоста и списком незапущенных сервисов на консоль и в файл
регистрации критичных уведомлений и завершается с кодом выхода 2.

При невыявлении недоступных критических сервисов, выводит информационное сообщение с адресом хоста и о том, что незапущенные сервисы 
отсутствуют.
'''
def service_stat(arg,host):
    ...
    logger.info('{host} - There are no offline services!')
    return 0

# Функция выявления доуступности SIP-trunkов.
'''
Сервис sipproxy
Информацию о наименовании и код статуса соединения хранит в текстовом файле.

Единственным аргументом является адрес сервера, для удобного отслеживания выполнения проверок на нескольких серверах.
...
При отсутсвия каналов с таким кодом - выводит критичное сообщение с адресом хоста и всеми каналами с кодом, отличающегося от отличного,
на консоль и регистрирует его в файл регистрации критичных уведомлений и завершается с кодом выхода 3.

При выявлении одного и более доступных SIP trunkов, в консоль и в файл регистрации выводит сообщение с адресом хоста и списком неработающих SIP trunkов.
...
'''
Получим список подключений на сервере с помощью команды : *sleep 1 | /opt/naumen/nauphone/bin/naucore show connections*
Информация записывается в переменную в виде списка.

Единственным аргументом является адрес сервера, для удобного отслеживания выполнения проверок на сервере.
Функция просматривает список и выполняет поиск, основанный на использовании регулярных выражений, ключа *client*,
...
При отсутсвии авторизованных пользователей - выводит критичное сообщение с адресом хоста на консоль и регистрирует его 
в файл регистрации критичных уведомлений и завершается с кодом выхода 1.

При выявлении одного и более авторизованных операторов, выводит информационное сообщение с адресом хоста и количеством 
